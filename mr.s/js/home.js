...
var MAC={
    ...
    'Gbook':{
        ...
        'Submit':function(){
            ...
            MAC.Ajax(maccms.path + '/index.php/gbook/saveData','post','json',$('.gbook_form').serialize(),function(r){
                MAC.Pop.Msg(100,20,r.msg,1000);
                if(r.code == 1){
	            	$('body').append('<div class="shortcuts-box" style="display: block;"><div id="shortcuts-info">感谢您的反馈，问题已提交</div></div>');
    ...
    'History': {
        ...
        'Init':function(){
            ...
            if($(".mac_history_set").attr('data-name')){
                ...
                MAC.History.Set($that.attr('data-name'),$that.attr('data-link'),$that.attr('data-list'));
            }
		...
		'Set':function(name,link,list){
			if(!link){ link = document.URL;}
			var history = MAC.Cookie.Get("mac_history");
		    var len=0;
		    var canadd=true;
		    if(history){
		        ...
		        $(history).each(function(){
		            if(name==this.name){
		                ...
		                $(history).each(function(i){
		                    ...
		                    json+="{\"name\":\""+temp_name+"\",\"link\":\""+temp_link+"\",\"list\":\""+temp_list+"\"}";
		                    if(i!=len-1)
		                    json+=",";
		                })
		                json+="]";
						MAC.Cookie.Set('mac_history',json, this.Days);
		                return false;
		            }
		        });
		    }
		    if(canadd) {
		        ...
		        var isfirst="]";
		        isfirst=!len?"]":",";
		        json+="{\"name\":\""+name+"\",\"link\":\""+link+"\",\"list\":\""+list+"\"}"+isfirst;
		        if(len>this.Limit-1)
	            	len-=1;
	        	for(i=0;i<len-1;i++){
	            	json+="{\"name\":\""+history[i].name+"\",\"link\":\""+history[i].link+"\",\"list\":\""+history[i].list+"\"},";
	       	 	}
	        	if(len>0){
	            	json+="{\"name\":\""+history[len-1].name+"\",\"link\":\""+history[len-1].link+"\",\"list\":\""+history[len-1].list+"\"}]";
	        	}
		        MAC.Cookie.Set('mac_history',json, this.Days);
		    }
		},
        'Clear': function(){
            ...
            $('.drop-history .drop-content-items').html('<li class="drop-tips">暂无观影历史</li>');
        },
    },
    'Ulog':{
        'Init':function(){
    ...
    'Pop':{
        ...
        'Notice':function($title,$msg) {
            $('body').append('<div class="popup open"><div class="popup-icon"><img src="/static/images/backhome.svg"></div><div class="popup-header"><h3 class="popup-title"></h3></div><div class="popup-main"></div><div class="popup-footer"><span class="popup-btn" id="close-popup">我记住啦</span></div></div><div class="shortcuts-mobile-overlay"></div>');
        ...
        'Show':function($w,$h,$title,$url,$callback) {
            ...
            $('body').append('<div class="popup" id="report-popup"><div class="popup-icon"><img src="/static/images/report.svg"></div><div class="popup-header"><h2 class="popup-title"></h2></div><div class="popup-main"></div><div class="close-popup" id="close-popup" ><i class="icon-close-o"></i></div></div><div class="shortcuts-overlay"></div>');
            $('#close-popup').click(function(){
