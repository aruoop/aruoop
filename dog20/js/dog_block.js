...
var baidushare = "0";
//是否开启百度分享，0关闭，启用本地分享填写https://cdn.jsdelivr.net/gh/aruoop/aruoop@master/dog20/api/js/share.js，启用官网分享填写http://bdimg.share.baidu.com/static/api/js/share.js（不支持https）
	...
	'mobile': {//移动端
		...
		'mshare': function() {
			$(".open-share").click(function() {
				dog.browser.useragent.weixin ? $("body").append('<div class="mobile-share share-weixin"></div>') : $("body").append('<div class="mobile-share share-other"></div>');
		...
		'history': function() {
			if($("#dog_history").length){
				if($.cookie("recente")){
				    ...
				    $("#dog_history").append(list);
				}
				else
		            $("#dog_history").append("<p style='padding: 80px 0; text-align: center'>您还没有看过影片哦</p>");
		...
		'copylink': function(){
			if(copyshort==1){
				if($(".copylink").length){
					$.ajax({ 
						type : 'GET',  
				        url : 'https://api.weibo.com/2/short_url/shorten.json?source=2849184197&url_long='+encodeURIComponent(dog.browser.url),
				        dataType : 'JSONP',   
				        success : function(r) {
...
$(document).ready(function() {	
	if(dog.browser.useragent.mobile){
		dog.mobile.slide();
		dog.mobile.popup();
		dog.mobile.mshare();
	}
	...
	dog.images.carousel();
	dog.images.qrcode();
	dog.common.bootstrap();
	dog.common.headroom();
	dog.common.history();
	dog.common.collapse();
	dog.common.scrolltop();
	dog.common.copylink();
	dog.common.share();
});
